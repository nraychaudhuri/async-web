...
object Application extends Controller {
  ...
  def feed(role: String, lowerBound: Int, higherBound: Int) = Action {
    ...
    val finalAdpater: Enumeratee[Event,JsValue] = secure.compose(inBounds).compose( asJson)
    
    Ok.feed(Stream.events.through( finalAdpater).through(EventSource())).as("text/event-stream")
